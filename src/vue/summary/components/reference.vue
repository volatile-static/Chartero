        ...
        async processReferenceNetwork(cancelToken?: { cancelled: boolean }) {
            function getAttachmentText(att: Zotero.Item) {
                if (cancelToken?.cancelled)
                    return '';  // 取消执行当前promise
                if (__test__)
                    return att.attachmentText;  // 测试环境不处理异常
                return att.attachmentText.catch(e => {
                    if (e.name == 'InvalidPDFException')
                        addon.log(`Invalid PDF: ${att.getField('title')}`);
                    else
                        addon.log(e, att);
                });
            }
            ...
            if (cacheKey in caches) {  // 缓存命中
                    ...
                    for (let j = i + 1; j < textList.length; ++j)
                        if (textList[i].includes(this.history[j].getField('title'))) {
                            ...
                            this.seriesData.push(
                                [this.history[i].id, this.history[j].id].map(String)
                            );
                        }
                ...
                if (prefs.length > 1024 * 1024)
                    prefs = JSON.stringify({});  // 超过1MB的缓存清空
...
<template>
    <Transition>
        <t-progress v-if="progress != 1" :percentage="percentage" status="active" theme="plump" class="progress" />
    </Transition>
